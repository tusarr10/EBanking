...
Public Class rdjournaltrns
    ...
    Private Sub InsertDataIntoSqltransctiondb(i As Integer)
        ...
        Dim x As Integer
        For x = 0 To i - 1
            JournalData = New rdJournalClass
            ...
            ASPxMemo1.Text = logmsg
            UPN1.Update()
            If Not DataService.IsTridExistInrdJournal(JournalData.trid) Then
                'TODO if transction Not Available then Insert Data
                Dim ii As Boolean
                ii = DataService.addTordjournal(JournalData)
                If ii Then
                    z += 1
            ...
            Else
                ...
                Dim iii As Boolean
                If Not DataService.IsDataExistInrd(JournalData) Then
                    'todo if data duplicate not found then do Insert Data
                    iii = DataService.addTordjournal(JournalData)
                    If iii Then
                        z += 1
    ...
    Private Sub LoadCiffromdb()
        Try
            datasetcifdb.Tables("Accessrdtr").Clear()
        Catch
        End Try
        Try
            ...
            Dim dataadapter1 As OleDbDataAdapter = New OleDbDataAdapter(New OleDbCommand("SELECT * FROM " & "Rdac" & "", databaseconnection1))
            dataadapter1.Fill(datasetcifdb, "Accessrdtr")
            'ShowData(currentrow)
    ...
    Function getCifAccessDataTable() As DataTable
        Try
            Return datasetcifdb.Tables("Accessrdtr")
        Catch ex As Exception
            Return Nothing
        End Try
    End Function
